...
.factory('fonts', ['$rootScope', '$http', '$timeout', '$filter', 'localStorage', function($rootScope, $http, $timeout, $filter, localStorage) {
    ...
    var fonts = {
        ...
        load: function(names) {
            ...
            head.append(
                '<link rel="stylesheet" id="dynamic-fonts" href="http://fonts.useso.com/css?family='+
        ...
        createLinkToFont: function(font) {
            ...
            if (link[0]) {
                link.attr('href', 'http://fonts.useso.com/css?family='+name);
            } else {
                $('head').append('<link rel="stylesheet" id="'+name+'" href="http://fonts.useso.com/css?family='+name+'">');
            }
        }
    };
